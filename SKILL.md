---
name: subatomizer
description: >-
  Atomiza un artículo ya publicado en cinco notas de Substack escritas con la voz del propio
  autor, destilada del artículo, y ancladas a fragmentos exactos del texto. Estudia cómo
  escribe quien lo firmó y qué material da el texto, escribe nueve candidatas, comprueba una
  por una, trayendo la frase del original que respalda cada cifra y cada cita, y entrega las
  cinco mejores —hasta dos de ellas marcadas como lectura del texto y no como el texto— más las
  cuatro descartadas con su motivo. Úsalo cuando alguien pase la URL de
  un artículo o de un post de Substack y quiera sacarle notas; cuando pida repartir, trocear,
  atomizar o reaprovechar algo que ya publicó; cuando quiera contenido para Notes a partir de
  su newsletter; o cuando pregunte qué notas salen de un texto suyo. Vale con Substack y con
  cualquier otra URL. La señal es que HAY un artículo del que anclar: si lo que hay es una idea
  suelta y ningún texto previo, eso es escribir una nota desde cero y no es este skill. Lo que
  no está en el artículo, no sale en la nota.
---

# subatomizer — cinco notas que el artículo ya contiene

Un modelo atomizando un artículo hace dos destrozos. Redondea la cifra, retoca la cita y se
inventa el ejemplo. Y escribe las notas con su propia voz de modelo, que suena a cualquiera.
Quien publica eso lo hace con su nombre encima.

Contra lo primero, una regla que no se salta: **de cada cifra y de cada cita hay que traer la
frase del artículo que la respalda**, y donde haya fichero se confirma con Grep. Contra lo
segundo, el propio artículo: lo escribió él, así que ahí está su voz.

Cinco fases: **traer el texto, estudiarlo, escribir nueve, verificarlas, elegir cinco.**

---

## 1. Traer el artículo y guardarlo

Baja el artículo con WebFetch, pidiéndolo literal:

> Devuelve el texto íntegro del artículo, literal y sin resumir: título, subtítulo y todos los
> párrafos en orden, con sus cifras, sus nombres y sus frases entrecomilladas tal cual están.
> No interpretes, no acortes, no reformules.

Guárdalo en `~/.claude/subatomizer/<fecha>-<slug>/articulo.txt`. **Ese fichero es el patrón de
medida**: contra él se comprueba todo en la fase 4.

Antes de seguir, míralo:

- **Menos de 300 palabras** — de ahí no salen cinco notas ancladas. Se dice y se para.
- **Corte de pago** — se avisa antes de las notas y se trabaja con lo visible.
- **Trescientas palabras y ninguna cifra en un artículo que va de números** — te devolvieron un
  resumen. Repite pidiendo el texto literal.

**Abre la página con un navegador** —`agent-browser`, la extensión de Claude en Chrome, un MCP
de Playwright— cuando el post sea de pago, cuando la página se monte con JavaScript, o cuando lo
recuperado siga oliendo a resumen. En Substack el cuerpo está en `.available-content`:

```bash
agent-browser open "<url>"
agent-browser get text ".available-content"
```

WebFetch devuelve una versión del artículo, no el artículo: lo pasa por un modelo pequeño. Basta
casi siempre, pero **dilo antes de las notas** —«lo que compruebo es esa versión, no tu texto»—
y no lo llames falta de acceso, que bajarlo lo bajaste.

Si no puedes escribir ficheros —claude.ai, el móvil— la fase 4 funciona igual: lo que no
tendrás es el Grep que la confirma, así que lo que no localices a la primera cae.

---

## 2. Estudiar el artículo

De leerlo salen dos cosas: **qué material da** y **cómo escribe quien lo firmó**. Sin lo primero,
las nueve notas salen del mismo tercio del texto. Sin lo segundo, suenan a modelo.

### El inventario

Rellena esta tabla **copiando literal**: aquí no se interpreta nada todavía.

| Qué | Cómo se recoge |
|---|---|
| **Tesis** | Una frase: qué defiende el artículo |
| **Ideas** | De tres a seis, una línea cada una |
| **Cifras** | Cada número, con la frase entera donde aparece |
| **Frases que aguantan solas** | Las citables, **literales**, con su puntuación |
| **Escenas** | Momentos concretos: qué pasó, cuándo, a quién |
| **Nombres** | Personas, productos y trabajos de otros que el texto cita |
| **Tensiones** | Lo que el artículo deja abierto, lo que se contradice, lo que incomoda |
| **Instrucciones** | Pasos o consejos que el lector puede ejecutar |

Una casilla vacía es información: si no hay cifras, no habrá nota de cifra. **Y si el inventario
sale casi vacío —un artículo de enlaces, una nota de prensa— dilo antes de escribir**: ese texto
da para dos o tres notas, no para cinco.

### El perfil de voz

Seis rasgos, y **cada uno con su prueba citada del artículo**. Un rasgo sin cita es una impresión
tuya, y las impresiones producen imitaciones de imitación:

| Rasgo | Qué mirar |
|---|---|
| **Persona y registro** | ¿Tutea, trata de usted, primera persona, plural? ¿Coloquial o formal? |
| **Largo de frase** | ¿Cortas y secas, o largas con subordinadas? Cuenta un par |
| **Léxico propio** | De cinco a diez expresiones suyas, **literales**. Las que no usaría otro |
| **Cómo abre** | ¿Por una escena, un dato, una afirmación, una pregunta? |
| **Cómo remata** | ¿Con una acción, una vuelta de tuerca, en seco, una pregunta? |
| **Puntuación y forma** | ¿Guiones largos, paréntesis, cursivas, listas? ¿Párrafos de una línea? |

Y una séptima cosa, la que más se nota cuando falta: **la lista anti-imitación**. Qué **no** hace
nunca, punto por punto. Si no usa emojis, no hay emojis. Si no usa superlativos, ninguna nota
lleva «increíble». Esta lista se usa dos veces: al escribir y al puntuar.

**El único ajuste permitido.** Un artículo se lee sentado y una nota bajando con el pulgar: se
conserva el registro, el léxico y la puntuación, y **se acorta la frase**. Nada más. Si de un
ensayista sale una nota que suena a community manager, la voz se perdió por el camino.

Enseña el perfil al usuario antes de escribir las notas. Si te equivocaste, lo ve en dos
segundos, y corregirlo ahí cuesta mucho menos que corregir nueve notas.

---

## 3. Escribir nueve candidatas

Nueve para tener de dónde elegir. Se entregan cinco. Cada una declara cinco cosas:

| Campo | Qué es |
|---|---|
| `formato` | Uno de los nueve de abajo |
| `objetivo` | Uno solo: enseñar, convencer, conectar, conversar o dar crédito |
| `tipo` | `anclada` o `inferida` |
| `texto` | La nota entera, tal como se publicaría |
| `ancla` | El fragmento **exacto** del artículo del que nace, copiado literal |

El `ancla` no es el tema de la nota ni su resumen: es el trozo de texto del que sale. «Habla del
alcance» no es un ancla; «el alcance cayó un 37% en marzo» sí. **Si no puedes señalar el
fragmento, no hay nota.**

### Los nueve formatos

| Formato | Qué es | Qué necesita del inventario |
|---|---|---|
| **arreglo** | Un problema concreto y su solución en sesenta segundos | Una instrucción ejecutable hoy |
| **historia-dato** | Un momento real pegado al número que lo mide | Una escena **y** una cifra. Con una sola, no es esto |
| **contraria** | Desmontar algo que el público da por sentado | Una tensión, y que la sostengas en los comentarios |
| **confesion** | Lo que salió mal, lo que costó, lo que da apuro | Una escena que le deje en mal lugar |
| **tras-bambalinas** | Cómo se hizo algo por dentro, con sus números | Proceso más cifras |
| **generosidad** | Señalar el trabajo de otro que el artículo cita | Un nombre. Lo que más convierte y lo que menos se usa |
| **pregunta** | Una pregunta genuina sobre lo que el texto deja abierto | Una tensión sin resolver |
| **sentencia** | Una frase del artículo que aguanta sola, comentada | Una frase citable. El formato más difícil de fingir |
| **lista-corta** | De tres a cinco puntos, una línea cada uno | Ideas de verdad enumerables |

Uno por candidata. **Si el inventario no da material para un formato, esa candidata no se
escribe**: ocho honestas valen más que nueve con una inventada.

### Anatomía

**50–250 palabras**, uno a cinco párrafos cortos, con blanco entre ellos.

1. **Gancho** — primera línea, máximo 120 caracteres, con una cifra, un nombre o un problema
   reconocible dentro. Ábrelo **como abre él**.
2. **Sustancia** — dos a cinco líneas: la historia, el dato o el arreglo.
3. **Cierre** — una acción que el lector pueda probar o una pregunta real. **Como remata él.**

### Ancladas e inferidas

Una **anclada** dice lo que el artículo dice, con otra forma. Una **inferida** afirma algo que el
artículo no llega a decir:

| Inferencia | Qué hace |
|---|---|
| **Consecuencia** | Lo que se sigue de sus propios datos |
| **Objeción** | La pregunta incómoda que el artículo no responde |
| **Límite** | Dónde deja de valer lo que defiende |
| **Pregunta abierta** | Lo que habría que saber y no se sabe |

Son las que generan respuestas: una anclada se lee y se asiente; una inferida se lee y alguien
contesta. Por eso entran, y por eso llevan tope: **hasta cuatro de las nueve, dos de las cinco.**

### Tres ejemplos

Del artículo:

> Publiqué 16 notas la semana pasada. Una me trajo 514 suscriptores y las otras quince no
> llegaron a veinte entre todas. No era la mejor escrita: era la única que contaba un número
> mío.

**1 · Anclada** — se entrega tal cual.
`historia-dato` · `enseñar` · ancla: «Una me trajo 514 suscriptores y las otras quince no
llegaron a veinte entre todas»

```
Publiqué 16 notas la semana pasada.

Una me trajo 514 suscriptores. Las otras quince, menos de veinte entre todas.

No era la mejor escrita. Era la única que contaba un número mío.
```

Cada palabra sale del fragmento, el gancho es una cifra suya y la nota se entiende sin haber
leído el artículo.

**2 · Inferida** — se entrega etiquetada.
`contraria` · `convencer` · misma ancla

```
Publiqué 16 notas la semana pasada. Una trajo 514 suscriptores. Las quince restantes, menos
de veinte entre todas.

Si una de dieciséis hizo casi todo el trabajo, publicar más no es la palanca.

La palanca es tener números propios que contar. Y esos no salen de escribir: salen de hacer
cosas y medirlas.
```

Las cifras son suyas; la conclusión es una lectura. Fíjate en «casi todo el trabajo» donde la
tentación era «el 96%»: el 96 es correcto —514 de unos 534— pero es una **cifra derivada**, no
está en el artículo y cae en la fase 4. La salida siempre es escribir la relación sin el número:
«una de dieciséis», «casi todo», «la quinta parte». Si el autor quiere el porcentaje, lo pone
él: es su cuenta y es su nombre.

**3 · Inventada** — no se escribe nunca.

```
¿Sabías que una sola nota puede cambiar tu Substack? 🚀

Publiqué más de 15 notas y una me trajo cientos de suscriptores, como le pasa al 80% de los
escritores. La clave está en la autenticidad.

Suscríbete para más consejos como este.
```

Seis líneas, cinco fallos: el 80% no existe; «más de 15» y «cientos» redondean lo que el texto
daba exacto; el emoji y la pregunta retórica no son suyos; «autenticidad» no aparece en el
artículo; y el cierre pide suscripción.

**La diferencia entre la 2 y la 3 es la única que hay que tener clara**: la inferida saca una
conclusión nueva de hechos viejos; la inventada mete un hecho nuevo. La conclusión la firma el
autor y se discute en los comentarios. El hecho falso no se discute: se desmiente.

### La prueba anti-slop

La lista anti-imitación caza lo que **él** no hace. Esta caza lo que hacen **todos los modelos**,
que se cuela porque suena bien.

**La prueba de portabilidad, primero.** ¿Podría esta frase estar en la nota de otra persona,
sobre otro artículo, sin cambiar una palabra? Si sí, es relleno: bórrala o cámbiala por un
hecho, un nombre, una cifra o un juicio que solo valga para este texto. Esta sola prueba tumba
la mitad del slop.

| Tic | Ejemplo |
|---|---|
| **Contraste de manual** | «No es X, es Y». «La pregunta no es X, sino Y» |
| **Enumeración negativa** | «No es una herramienta. No es un truco. Es un método» |
| **El trío** | Tres elementos cuando el material daba dos o cuatro |
| **Falsa confidencia** | «Lo que nadie te cuenta», «lo que casi todos se saltan» |
| **Revelación con dos puntos** | «Y lo que lo hace funcionar: otro agente lo corrige» |
| **Adorno de apertura** | «En un mundo donde…», «Todos hemos pasado por…» |
| **Conector de relleno** | «La realidad es que», «en definitiva», «al final del día» |
| **Elogio vacío** | «potente», «brutal», «clave», «esencial», «revolucionario» |
| **Puffery de importancia** | «Marca un antes y un después», «lo cambia todo» |
| **Atribución sin nombre** | «Los expertos dicen», «está demostrado», «todo el mundo sabe» |
| **Pregunta retórica que te respondes** | «¿Y sabes qué pasó? Pues esto» |
| **Aforismo final** | Una frase profunda, un lazo que repite la apertura, o «Punto.» |
| **Rotación de sinónimos** | Lo mismo llamado «la herramienta», «el sistema» y «la solución» |

### Las nueve juntas

- **Nueve anclas distintas.** Dos notas del mismo fragmento son la misma nota escrita dos veces.
- **Como mucho dos candidatas por objetivo.**
- **¿Se parecen entre sí?** Si las nueve tienen el mismo esqueleto —gancho corto, tres líneas,
  remate seco— el esqueleto es del modelo, no suyo. Rompe al menos tres.
- **¿Hay alguna frase que él no podría haber escrito?** Táchala aunque sea la mejor de la nota.

Cuando tumbes una nota por esto, **nombra el tic y cita la línea** —«aforismo final: *y eso lo
cambia todo*»—. Un veredicto sin cita no se puede comprobar, y el autor tiene que poder llevarte
la contraria.

---

## 4. Verificar

Releer una nota que te acabas de inventar no te dice que te la inventaste: te sigue pareciendo
bien. Así que no se relee. **De cada cifra y de cada frase entrecomillada, se trae la frase
entera del artículo donde aparece.**

No «sí, el 514 está». Esto:

> 514 → «Una me trajo 514 suscriptores y las otras quince no llegaron a veinte entre todas»

Inventarse un número es fácil; inventarse la frase que lo rodea, con el párrafo delante, es
mucho más difícil y se ve enseguida. Si no puedes traer la frase, la nota cae.

Donde haya fichero, **confírmalo con Grep**, que no tiene memoria ni orgullo:

```
Grep  pattern: \b514\b                     path: articulo.txt
Grep  pattern: me trajo 514 suscriptores   path: articulo.txt
```

Las cifras con `\b`, o «514» pasaría por estar dentro de «5140». Las frases, por un trozo de
cuatro a seis palabras sin comillas ni puntuación: es lo que evita que una cita correcta caiga
por un carácter tipográfico.

Y una cautela que salió de una ejecución real: **cero resultados y herramienta rota no son lo
mismo**. Si caen varias a la vez, busca algo que sepas que está —el título sirve—; si eso tampoco
aparece, lo roto es la búsqueda. Con la herramienta sana, cero resultados es una caída y no se
«ajusta la búsqueda hasta que salga».

### Ocho motivos de caída

| Fallo | Qué lo dispara |
|---|---|
| **ancla-existe** | El `ancla` no aparece en el artículo |
| **citas-literales** | Una frase entrecomillada no está literal en el artículo |
| **cifras-reales** | Un número con decimales, %, moneda o unidad detrás —o cualquier entero mayor que diez— que no está en el artículo. Incluye las **derivadas**, las que calculaste tú |
| **hecho-nuevo** | Afirma como cierto algo que el artículo no dice: una estadística, lo que hace «la gente», lo que pasa «siempre». Es el único freno de las inferidas |
| **longitud** | Menos de 50 o más de 250 palabras |
| **gancho** | La primera línea pasa de 120 caracteres |
| **sin-enlaces** | Hay una URL o un dominio dentro de la nota |
| **sin-captación** | «Suscríbete», «sígueme», «enlace en bio» o equivalente |

`hecho-nuevo` es el único sin frase de respaldo que traer: exige preguntarte «¿esto lo dice el
artículo o lo estoy dando por sabido?». En la duda, cae.

**Aviso, no caída** — `cifras-menores`: un entero de uno a diez que no está en el artículo. Casi
siempre es legítimo («van 3 cosas que aprendí» cuenta los puntos de la propia nota), pero si
llega hasta la entrega **se dice nombrando el número**.

La nota que cae se arregla y se vuelve a verificar. No se pasa a la selección con fallos
pendientes, ni se entrega «avisando de que igual el dato no está».

---

## 5. Elegir cinco

Solo entre las que pasaron. Si una te gusta y falló, arréglala, verifícala y entonces compite.

Seis ejes, de 0 a 3. Máximo 18.

| Eje | 3 puntos | 0 puntos |
|---|---|---|
| **Voz** | No incumple ni un punto de las listas anti-imitación y anti-slop, y usa léxico suyo | Incumple dos o más, o no hay una palabra que sea suya |
| **Ancla** | Un hecho duro: cifra, nombre, frase literal | Una generalidad que saldría de cualquier artículo |
| **Autonomía** | Se entiende sin haber leído el artículo | Solo tiene sentido si ya lo leíste |
| **Gancho** | La primera línea dice algo concreto | Podría encabezar otra nota cualquiera |
| **Conversación** | Alguien tendría algo que responder | Solo cabe asentir |
| **Utilidad** | El lector se lleva algo hoy | El lector se lleva una impresión |

La voz **se puntúa contando incumplimientos**, no por impresión: las escribiste tú, así que tu
impresión siempre te dará un 3. Y va primera porque una nota impecable que no suena a él no la va
a publicar, y si la publica, le resta.

**Tres restricciones, por encima de la puntuación:**

- Cinco objetivos distintos.
- Como mucho dos del mismo formato.
- Como mucho dos inferidas — tres ancladas es el suelo.

Una nota de 16 sobre 18 se queda fuera si su objetivo ya está cogido: entra la siguiente. Y si el
artículo no daba para inferir bien, salen cinco ancladas: el tope es un techo, no una cuota.

### Cómo se entrega

1. El aviso de truncado, si lo hubo.
2. **El perfil de voz**, en cinco o seis líneas.
3. Las cinco, **cada una en su propio bloque de código**, de una en una y no las cinco juntas.
   Dentro del bloque va **solo el texto de la nota**: sin título, sin comillas que la envuelvan,
   sin numeración y sin markdown —Notes no lo interpreta y se pega como basura—.

   Encima del bloque, nunca dentro: formato, objetivo y **por qué está esa y no otra** —el
   motivo, no el elogio: «es la única con una cifra propia», no «muy potente»—. Y si es
   **inferida**, ahí mismo: *«esto es una lectura tuya del texto, no algo que el artículo diga»*.
   Eso no se omite ni se suaviza aunque la nota te guste: quien la firma tiene que saberlo.
4. Las cuatro descartadas, una línea con su motivo real: «cayó por cita no literal», «el ancla se
   repetía con la 3», «16 sobre 18 pero el objetivo ya estaba cogido». Ahí es donde el autor
   rescata la que le duela perder.
5. Los avisos que quedaran vivos, nombrando el número.

El `ancla` no se enseña salvo que la pidan: es la costura, no el producto.

---

## Lo que no se hace

- **No se pregunta qué tono quiere.** Está en el artículo. Preguntarlo es admitir que no lo has
  leído.
- **No se rellena hasta cinco.** Si pasan cuatro, se entregan cuatro y se dice por qué. El cinco
  no es un compromiso con el usuario; que ninguna nota lleve un dato falso, sí.
- **No se enseña una nota que no pasó la verificación**, ni «de muestra».
- **No se escribe desde la memoria del artículo.** Si no se pudo traer, no hay notas: se dice.

---

## De dónde salen estas reglas

Formatos, longitud y anatomía vienen de tres fuentes que miden, consultadas el 2026-09-14:
`thrivewithcarrie.substack.com/p/substack-notes-strategy-2026` (los formatos que convierten, y
que un artículo de dos mil palabras da para tres a cinco notas),
`pubstacksuccess.substack.com/p/how-to-write-high-performing-substack-notes` (50–250 palabras) y
`writebuildscale.substack.com/p/the-2026-substack-notes-playbook` (un objetivo por nota). Se deja
fuera a conciencia el repertorio de curiosidad que esas mismas fuentes recomiendan.

La prueba anti-slop debe bastante a **no-ai-slop**, de Peter Yang
(`github.com/petergyang/no-ai-slop`): la prueba de portabilidad y la idea de nombrar el patrón y
citar la línea en vez de puntuar. Para un texto largo, ese skill es mejor herramienta que este.

Con **subnotes** instalado hay un salto más: su histórico dice qué formatos le funcionan **a él**
con sus propios números, y avisa de si ya había dicho esto. Aquí no se asume que esté.
